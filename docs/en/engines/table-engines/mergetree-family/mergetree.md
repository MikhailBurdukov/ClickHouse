...
- `move_policy` - Policy for selecting parts for move to the next volume with the total size that is sufficient to meet the `move_factor` condition:
    - `by_part_size` - sorts existing parts by size from largest to smallest (in descending order).
    - `experimental_by_insert_data_time` -  sorts existing parts by time of data insert in this part(older parts first). The policy are experimental and works as intended only with another expiremental option `allow_generate_min_max_data_insert_file`, which allows to generate `min/max_data_insert.txt` file. By now there are backoffs, if the `allow_generate_min_max_data_insert_file` is disabled, but when the feature gets out of the experimental stage, they will be removed.
