...
{
        priorities_holder.insert({SegmentType::Data, creator_function(max_data_segment_size, max_data_segment_elements, size_ratio, description_ + "_" + getKeyTypePrefix(SegmentType::Data))});
        priorities_holder.insert({SegmentType::System, creator_function(max_system_segment_size, max_system_segment_elements, size_ratio, description_ + "_" + getKeyTypePrefix(SegmentType::System))});
}
...
bool SplitFileCachePriority::modifySizeLimits(size_t max_size_, size_t max_elements_, double size_ratio_, const CachePriorityGuard::Lock & lock)
{
    ...
    priorities_holder.at(SegmentType::Data)->modifySizeLimits(max_data_segment_elements, max_data_segment_size, size_ratio_, lock);
    priorities_holder.at(SegmentType::System)->modifySizeLimits(max_data_segment_elements, max_data_segment_size, (1-size_ratio_), lock);
    return true;
}
...
{
    if (best_effort)
        return priorities_holder.at(SegmentType::System)->canFit(size, elements, lock, reservee, best_effort)
            || priorities_holder.at(SegmentType::Data)->canFit(size, elements, lock, reservee, best_effort);

    if (reservee)
    {
        const auto * split_iterator = assert_cast<SplitIterator *>(reservee.get());
        return priorities_holder.at(split_iterator->type)->canFit(size, elements, lock, reservee, best_effort);
    }
    // No info about segment key type; put it into data segment;
    return priorities_holder.at(FileSegmentKeyType::Data)->canFit(size, elements, lock, reservee, best_effort);
}
